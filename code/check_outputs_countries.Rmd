...
temp_file <- 'output/jpn_prefecture_era5_temp_average_2009_2018_polynomial_5_area_crop_weights.csv'
...
map_area_weights <- ggplot() + 
  ...
  geom_sf(data = country_shp, fill = NA, lwd = 0) + 
...
ggsave(weight_fig,
       filename = file.path(paste0(save_path, 'weights_fig.pdf')),
       dpi = 300,
       device = 'pdf')
...
ggsave(poly1_plot, filename = paste0(save_path, 'temp/poly1_temp.pdf'))
...
ggsave(poly2_plot, filename = paste0(save_path, 'temp/poly2_temp.pdf'))
...
ggsave(poly3_plot, filename = paste0(save_path, 'temp/poly3_temp.pdf'))
...
map_order1_feb <- low_temp_df %>% 
  ...
  labs(fill = 'Avg Feb\nTemperature\n(1st Order / Days in Month)',
       ...
       x = '',
       y = '') + 
  scale_fill_viridis_c(limits = c(round(min(low_temp_df$order_1_avg)), max(low_temp_df$order_1_avg))) + 
  facet_wrap(~year) + 
  theme_bw() 

ggsave(map_order1_feb, filename = paste0(save_path, 'temp/low_temp_out.pdf'))

# Map first order values for high temp month (July)
high_temp_df <-  out3_sf %>%
  filter(month == 'month_07')

map_order1_jul <- high_temp_df %>%
  ggplot() + 
  geom_sf(aes(fill = order_1_avg), lwd = 0) + 
  labs(fill = 'Avg July\nTemperature\n(1st Order / Days in Month)',
       title = 'area + crop weights',
       x = '',
       y = '') + 
  scale_fill_viridis_c(limits = c(round(min(high_temp_df$order_1_avg)), max(high_temp_df$order_1_avg))) + 
  facet_wrap(~year) + 
  theme_bw() 

ggsave(map_order1_jul, filename = paste0(save_path, 'temp/high_temp_out.pdf'))
...
map_order1_spring <- spring_df %>%
  ...
  labs(fill = 'Avg spring\nTemperature\n(1st Order / Days in Month)',
       ...
       x = '',
       y = '') + 
  scale_fill_viridis_c(limits = c(round(min(spring_df$order_1_avg)), max(spring_df$order_1_avg))) + 
  facet_grid(year~month) + 
  theme_bw() 

ggsave(map_order1_spring, filename = paste0(save_path, 'temp/spring_temp_out.pdf'))
...
map_order1_fall <- fall_temp_df %>%
  ...
  labs(fill = 'Avg fall\nTemperature\n(1st Order / Days in Month)',
       ...
       x = '',
       y = '') + 
  scale_fill_viridis_c(limits = c(round(min(fall_temp_df$order_1_avg)), max(fall_temp_df$order_1_avg))) +  
  facet_grid(year~month) + 
  theme_bw() 

ggsave(map_order1_fall, filename = paste0(save_path, 'temp/fall_temp_out.pdf'))
...
map_prcp_low <- low_prcp %>% 
  ...
  scale_fill_viridis_c(option = 'mako', direction = -1,
                       limits = c(round(min(low_prcp$order_1)), max(low_prcp$order_1))) + 
  facet_wrap(~year) + 
  theme_bw() 

ggsave(map_prcp_low, filename = paste0(save_path, 'prcp/low_prcp_out.pdf'))
...
map_prcp_high <- high_prcp %>% 
  ...
  labs(fill = 'Total June\nPrecipitation (mm)\n(1st Order)',
       ...
       y = '') + 
  scale_fill_viridis_c(option = 'mako', direction = -1,
                       limits = c(round(min(high_prcp$order_1)), max(high_prcp$order_1))) + 
  facet_wrap(~year) + 
  theme_bw() 

ggsave(map_prcp_high, filename = paste0(save_path, 'prcp/high_prcp_out.pdf'))
...
map_order1_wet <- wet_months %>% 
  ...
  labs(fill = 'Monthly Precipitation (mm)\nWet Season\n(1st Order)',
       ...
       y = '') + 
  scale_fill_viridis_c(option = 'mako', direction = -1,
                       limits = c(round(min(wet_months$order_1)), max(wet_months$order_1))) +  
  facet_grid(year~month) + 
  theme_bw() 

ggsave(map_order1_wet, filename = paste0(save_path, 'prcp/wet_season_out.pdf'))
...
map_order1_dry <- dry_months %>% 
  ...
  labs(fill = 'Total Precipitation (mm)\nDry Season\n(1st Order)',
       ...
       y = '') + 
  scale_fill_viridis_c(option = 'mako', direction = -1,
                       limits = c(round(min(dry_months$order_1)), max(dry_months$order_1))) +  
  facet_grid(year~month) + 
  theme_bw() 

ggsave(map_order1_dry, filename = paste0(save_path, 'prcp/dry_season_out.pdf'))
...
ggsave(poly1_plot_p, filename = paste0(save_path, 'prcp/poly1_prcp.pdf'))
...
ggsave(poly2_plot_p, filename = paste0(save_path, 'prcp/poly2_prcp.pdf'))
...
ggsave(poly3_plot_p, filename = paste0(save_path, 'prcp/poly3_prcp.pdf'))
