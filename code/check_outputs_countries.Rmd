...
## map eu
test <- st_shift_longitude(country_shp)

eu_map <- 
  ggplot() + 
  geom_sf(data = test, fill = "grey", lwd = 0) +
  xlim(min(st_coordinates(test)[, 1]) - 1, max(st_coordinates(test)[, 1]) +
             1) +
  ylim(min(st_coordinates(test)[, 2]) - 1, max(st_coordinates(test)[, 2]) +
             1)

ggsave(map_area_weights,
       filename = file.path(paste0(save_path, "/", country_name, '/eu_map.pdf')),
       dpi = 300,
       device = 'pdf')
