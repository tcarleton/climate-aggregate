...
era5_daily <- read_csv(file.path(here::here(), "data", "output",
                                   "ca_counties_test_2010.csv"))
...
era5_monthly_by_grid <- era5_daily %>% 
...
# February 
era5_feb <- era5_monthly_by_grid %>% 
  filter(month == 'month_02')

map_era5_feb <- ggplot() + 
  geom_raster(data = era5_feb,
...
# July 
era5_jul <- era5_monthly_by_grid %>% 
  filter(month == 'month_07')

map_era5_jul <- ggplot() + 
  geom_raster(data = era5_jul,
              aes(x=x, y=y, fill = avg_temp)) + 
  geom_sf(data = ca_counties, fill=NA, color='black') + 
  labs(fill = 'Avg July\nTemperature\n(Raw ERA5)',
       title = 'Montly ERA5 (2010)',
